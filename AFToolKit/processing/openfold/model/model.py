...
class AlphaFold(nn.Module):
    ...
    def forward(self, batch):
        ...
        # Run auxiliary heads
        for i in range(len(cycle_output)):
            cycle_output[i].update(self.aux_heads(cycle_output[i]))

        outputs.update(self.aux_heads(outputs))

        return outputs, cycle_output


class AlphaFoldRosettaHead(nn.Module):
    def __init__(self, afold_model):
        super(AlphaFoldRosettaHead, self).__init__()
        self.afold_model = afold_model
        self.head_pair = nn.Linear(128, 19)  # 512,1)
        self.head_single = nn.Linear(384, 1)
        self.layer_norm_pair = nn.LayerNorm(128)
        self.layer_norm_single = nn.LayerNorm(384)

    def forward(self, batch):
        predictions = self.afold_model(batch)
        pair_representations = predictions["pair"]
        single_representations = predictions["single"]
        predictions["energy_pair"] = self.head_pair(self.layer_norm_pair(pair_representations))
        predictions["energy_single"] = self.head_single(self.layer_norm_single(single_representations))
        return predictions
