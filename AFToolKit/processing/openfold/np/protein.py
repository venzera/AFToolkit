...
def from_pdb_df(pdb_df, chain_id: Optional[str] = None) -> Protein:
    ...
    for r, residue in  pdb_df.groupby(['residue_number', 'residue_name', 'chain_id'], sort=False):
        residue_name = r[1]
        res_shortname = residue_constants.restype_3to1.get(residue_name, "X")
        ...
        for atom in residue.iloc():
             atom_name = atom["atom_name"].replace(" ","")
             if atom_name not in residue_constants.atom_types:
                 print("atom name not found", atom_name)
                 exit(0)
                 continue
             pos[residue_constants.atom_order[atom_name]] = atom[  ["x_coord","y_coord","z_coord"]].to_numpy()
        ...
        residue_index.append(atom["residue_number"]-1)
