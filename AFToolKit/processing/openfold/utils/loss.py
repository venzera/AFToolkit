...
from AFToolKit.processing.openfold.utils.tensor_utils import (masked_mean, permute_final_dims,
...
def experimentally_resolved_loss(
    ...
    print(loss.shape)
    # print(eps.shape)
    print(atom37_atom_exists.shape)
    d = eps + torch.sum(atom37_atom_exists, dim=(-1, -2))
    print(d.shape)
    print(loss / d)
    exit(0)
...
class AlphaFoldLoss(nn.Module):
    ...
    def forward(self, out, batch, _return_breakdown=False):
        ...
        for loss_name, loss_fn in loss_fns.items():
            ...
            # print(loss)
            if (torch.isnan(torch.sum(loss)) or torch.isinf(torch.sum(loss))):
